...
- **`examples/gas_compute.py`** — ideal-gas box-bounce demo ported to a Phase-2 compute kernel. First example to use vec3 swizzle inside a kernel: per-axis bounce conditions read individual components (`if next_pos.x < -self.half_size`). Pairwise elastic collisions from `gas.py` are intentionally omitted — they need either atomics or a serialized impulse pass to avoid GPU-thread races.
